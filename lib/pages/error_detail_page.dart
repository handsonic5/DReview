...
class ErrorDetailPage extends StatefulWidget {
  const ErrorDetailPage({super.key});
...
class _ErrorDetailPageState extends State<ErrorDetailPage> {
  ...
  ErrorQuestion _error = ErrorQuestion();
  String _subject = '';
  bool _isNew = true;
  ...
  Future<void> _loadError() async {
    ...
    if (args != null) {
      ...
      if (!_isNew) {
        ...
        if (id != null) {
          final error = await _dataManager.getErrorById(id);
          if (error != null) {
            setState(() {
              _error = error;
              _titleController.text = error.title;
              _contentController.text = error.content;
            });
          }
        }
      } else {
        setState(() {
          _error = _error.copyWith(subject: _subject);
        });
      }
    }
  }
  ...
  Future<void> _saveError() async {
    if (_titleController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请输入错题标题')),
      );
      return;
    }
    ...
    final success = await _dataManager.saveError(_error);
    if (!mounted) return;
  ...
  Future<void> _askAi() async {
    ...
    setState(() {
      _isAiLoading = false;
      _error = _error.copyWith(aiResponse: response);
      _aiQuestionController.clear();
    });

    // 自动保存AI回复
    await _dataManager.saveError(_error);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _isNew ? '新建错题 - $_subject' : '编辑错题 - $_subject',
          maxLines: 1,

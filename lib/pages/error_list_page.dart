...
class _ErrorListPageState extends State<ErrorListPage> {
  ...
  String _subject = '';
  List<ErrorQuestion> _errors = [];
  List<ErrorQuestion> _filteredErrors = [];
  String _searchText = '';
  ...
  Future<void> _loadErrors() async {
    final errors = await _dataManager.getErrorsBySubject(_subject);
    // 按创建时间降序排序（最新的在最前面）
    errors.sort((a, b) => b.createTime.compareTo(a.createTime));
    setState(() {
      _errors = errors;
      _filterErrors();
    });
    print('错题列表已刷新，科目: $_subject, 当前数量: ${errors.length}');
  }
  ...
  // 获取日期分组
  Map<String, List<ErrorQuestion>> _getDateGroups() {
    final groups = <String, List<ErrorQuestion>>{};
    for (final error in _filteredErrors) {
      final dateKey = _formatDate(error.createTime);
      groups.putIfAbsent(dateKey, () => []).add(error);
    }
    return groups;
  }

  Future<void> _deleteError(String id) async {
    final success = await _dataManager.deleteError(id);
    if (!mounted) return;
          ...
          TextButton(
            onPressed: () async {
              await Navigator.pushNamed(
                context,
                '/error-detail',
                arguments: {'subject': _subject, 'isNew': true},
              );
              _loadErrors();
            },
            child: const Text(
        ...
        children: [
          ...
          Expanded(
            child: _filteredErrors.isEmpty
                ? Center(
                    child: Text(
                      _searchText.isNotEmpty ? '未找到相关错题' : '暂无错题，点击右上角添加',
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final errors = dateGroups[dateKey]!;
